...
################ Script Variables ###############################
LOGPATH="${TMPDIR}rinker.log"         # Log file

TMPIP=`mktemp -q "${TMPDIR}"rinker-ip.XXXXXX`   # Temp IP storage file
if [ $? -ne 0 ]; then
  echo "$0: Can't create temp IP address storage file, exiting..."
  exit 1
fi

TEMP=`mktemp -q "${TMPDIR}"rinker-temp.XXXXXX` # Temp storage file
if [ $? -ne 0 ]; then
  echo "$0: Can't create temp storage file, exiting..."
  exit 1
fi
...
# changeip
# URL=\"'https://nic.changeip.com/nic/update?cmd=update&set=$HOST&ip=$IP'\"

# google
# URL=\"'https://domains.google.com/nic/update?hostname=$HOST&myip=$IP'\"

# opendns
# URL=\"'https://updates.opendns.com/nic/update?hostname=$HOST'\"

IPPATH="${TMPDIR}rinker-ip-storage-${1}"     # IP address storage file
USER=$(eval "echo $2")
PASS=$(eval "echo $3")
HOST=$(eval "echo $4")
URL=$(eval "echo $5")
...
if diff "${IPPATH}" "${TMPIP}" > /dev/null
  then                                # same IP, no update
      ...
      wget -q -U "$AGENT" -O /dev/null --no-check-certificate --http-user="${USER}" --http-password="${PASS}" "${URL}"
      if [ $LOGLEVEL -ne 0 ]
