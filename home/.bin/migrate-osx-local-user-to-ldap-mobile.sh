	...
	select user in $listUsers; do
		...
		if [ "$user" = "FINISHED" ]; then
			...
			/usr/bin/dscl . -delete "/Users/$user"
				...
				if [ -f $homeDir/$netname ]; then
                    echo "Oops, theres a home folder there already for $netname."
                    echo "If you don't want that one, delete it in the Finder first,"
                    echo "then run this script again."
					exit 1
				else
                    /System/Library/CoreServices/ManagedClient.app/Contents/Resources/createmobileaccount -n $netname
                    /bin/rm -rf $homeDir/$netname
					/bin/mv $homeDir/old_$user $homeDir/$netname
					/usr/sbin/chown -R ${netname} $homeDir/$netname
					echo "Home for $netname now located at $homeDir/$netname"
				...
				select yn in "Yes" "No"; do
    					case $yn in
        					Yes) /usr/sbin/dseditgroup -o edit -a "$netname" -t user admin;
        					    echo "Admin rights given to this account";
        					    break;;
        					No ) echo "No admin rights given";
        					    break;;
    					esac
				done
			break
