...
if [ -f /etc/nfs_automount.conf ]; then
  CONFIG_FILE=/etc/nfs-automount.conf
elif [ -d "$HOME" ]; then
  CONFIG_FILE=$HOME/.nfs-automount.conf
fi
...
function log {
  ...
  if [ "${DEBUGLOG}" = "true" ] || [ "${critical}" = "true" ] ; then
    message=`echo "nfs-automount [${DATESTAMP}]: ${log_msg}"`
  ...
  if [ ${TAG} = "[CRIT]" ] ; then
    logger_msg=`echo "nfs-automount: ${log_msg}"`
    logger ${logger_msg}
  fi
}
...
if [ $(uname) = "Darwin" ]; then
    check_command ggrep; grepcmd=${_RET}
else
...
function valid_for_mount {
  ...
  # http://superuser.com/questions/467056/is-there-a-mountpoint-program-in-os-x
  # todo: use `diskutil info` on osx
  if [ $(uname) = "Darwin" ]; then
    _RET=true
  else
...
function nfs_umount {
  ...
  if ${_RET} ; then
    ...
    sudo ${umountcmd} -f -l "${_localmountpoint}"
    check_mounted "${_localmountpoint}"
    if ${_RET} ; then
      _RET=false
    else
  ...
  if ${_RET} ; then
    ...
    if ${_RET} ; then
      ...
      if ${_RET} ; then
        ...
        sudo ${mountcmd} ${_mountopts} ${_remotesystem}:${_remoteshare} ${_localmountpoint}
        if [ $? -ne 0 ] ; then
