...
function <SID>OpenSpecial(ochar,cchar) " ---{{{2
    if &filetype == 'handlebars'
        if a:ochar == '{'
            return a:ochar.a:ochar.<SID>CloseStackPushDoubleCurly(a:cchar)
        endif
    endif
    let line = getline('.')
    let col = col('.') - 2
...
function <SID>CloseStackPushDoubleCurly(char) " ---{{{2
    "echom 'push'
    let line = getline('.')
    let col = col('.')-2
    if (col) < 0
        ...
        "echom string(col).':'.line[:(col)].'|'.line[(col+1):]
        call setline('.',line[:(col)].a:char.a:char.line[(col+1):])
    endif
